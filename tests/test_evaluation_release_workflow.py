...
def test_evaluation_workflow_is_scoped_to_the_evaluation_fork() -> None:
    ...
    assert "  push:\n    branches: [patch/evaluation-upgrade-608]\n" in text
...
def test_every_job_binds_this_exact_evaluation_branch_and_source() -> None:
    text = WORKFLOW.read_text()
    jobs = [workflow_job(text, name) for name in ("authority", "compile", "sign")]

    assert "  EVALUATION_BRANCH: patch/evaluation-upgrade-608\n" in text
    assert "evaluation_branch:\n        description:" in text
    assert "source_sha:\n        description:" in text
    for job in jobs:
        assert 'test "$GITHUB_REF" = "refs/heads/$EVALUATION_BRANCH"' in job
        assert 'test "$DISPATCH_BRANCH" = "$EVALUATION_BRANCH"' in job
        assert '[[ "$DISPATCH_SOURCE_SHA" =~ ^[0-9a-f]{40}$ ]]' in job
        assert 'push) test -z "${DISPATCH_BRANCH:-}${DISPATCH_SOURCE_SHA:-}" ;;' in job
        assert "*) exit 64 ;;" in job
    assert 'test "$DISPATCH_SOURCE_SHA" = "$GITHUB_SHA"' in jobs[0]
    assert 'test "$DISPATCH_SOURCE_SHA" = "$SOURCE_SHA"' in jobs[1]
    assert 'test "$DISPATCH_SOURCE_SHA" = "$SOURCE_SHA"' in jobs[2]


def test_workflow_uses_the_explicit_evaluation_sequence() -> None:
    text = WORKFLOW.read_text()
    helper = HELPER.read_text()

    assert "EVALUATION_SEQUENCE = 609" in helper
    assert "EVALUATION_SEQUENCE = 558" not in helper
    assert "~dev.558" not in text and "~dev.558" not in helper
    assert "sequence=558" not in text and "sequence=558" not in helper
    assert '"sequence=609" \\' in text
    assert 'test "$version" = "${semantic}~dev.609+g${GITHUB_SHA:0:12}"' in text
...
def test_evaluation_workflow_compiles_native_arm64_with_pinned_toolchain() -> None:
    ...
    assert "rustup toolchain install 1.98.1" in compile_job
    ...
    assert "compiled/binaries/vonk-monitor" in compile_job
    assert "compiled/binaries/vonk-runtime-probe" in compile_job
    assert "prebuilt/binaries/vonk-monitor" in sign
    assert "prebuilt/binaries/vonk-runtime-probe" in sign
    assert r"printf 'vonk-agent-build-v2\0%s\0%s\0%s\0%s'" in compile_job
    assert (
...
IMAGES = ROOT / ".github/workflows/evaluation-controller-images.yml"
CANARY = ROOT / ".github/workflows/evaluation-local-canary.yml"
RECIPE_PIN = ROOT / "tests/acceptance/evaluation-recipe-library-revision.txt"
OFFICIAL_RECIPE_PIN = ROOT / "tests/acceptance/recipe-library-revision.txt"
INSTALLER_KEY = ROOT / "install/installer-release-public.pem"


def _pinned_actions(text: str) -> list[str]:
    return [
        match.group(1)
        for match in re.finditer(r"^\s*uses:\s*(\S+)", text, re.MULTILINE)
        if not match.group(1).startswith("./")
...
def test_evaluation_image_workflow_binds_current_image_contracts() -> None:
    text = IMAGES.read_text()

    assert "EVALUATION_BRANCH: patch/evaluation-upgrade-608" in text
    assert "kelchm/vonk-forge" in text
    assert "scripts/verify-public-image-inputs" in text
    assert "scripts/dev-image-inputs prepare" in text
    assert "VONK_SOURCE_COMMIT=${{ github.sha }}" in text
    assert "image-contexts/hermes/Dockerfile" in text
    assert "image-contexts/litellm/deploy/compose/litellm/Dockerfile" in text
    assert "ghcr.io/kelchm/vonk-forge-evaluation-${{ matrix.role }}" in text
    assert "ghcr.io/carstvaartjes/vonk-forge-" not in text
    assert _pinned_actions(text) == []


def test_evaluation_canary_uses_current_fresh_schema_native_acceptance() -> None:
    ...
    assert "python -m tests.acceptance.evaluation_local_release run" in text
    assert "VONK_EVALUATION_DISPOSABLE" in text
    assert "VONK_ACCEPTANCE_SPARK_CONTROLLER_BOUNDARY: loopback" in text
    assert "CarstVaartjes/vonk-forge-recipes" in text
    assert pin == "fe26bfe8070461d94ec476c42ca2060f74ef5143\n"
    assert pin != OFFICIAL_RECIPE_PIN.read_text()
    assert "tests/acceptance/recipe-library-revision.txt" not in text
    assert "tests/acceptance/evaluation-recipe-library-revision.txt" in text
    assert "alembic" not in text
    assert "0000_fresh_schema" not in text
    assert _pinned_actions(text) == []


def test_evaluation_installer_public_key_is_the_existing_fork_authority() -> None:
    text = INSTALLER_KEY.read_text()
