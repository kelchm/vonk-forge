...
def test_evaluation_workflow_is_scoped_to_the_evaluation_fork() -> None:
    ...
    assert "  push:\n    branches: [patch/evaluation-runtime-argument-budget]\n" in text
...
def test_every_job_binds_this_exact_evaluation_branch_and_source() -> None:
    ...
    assert "  EVALUATION_BRANCH: patch/evaluation-runtime-argument-budget\n" in text
...
def test_workflow_uses_the_explicit_evaluation_sequence() -> None:
    ...
    assert "EVALUATION_SEQUENCE = 556" in helper
    ...
    assert '"sequence=556" \\' in text
    assert 'test "$version" = "${semantic}~dev.556+g${GITHUB_SHA:0:12}"' in text
