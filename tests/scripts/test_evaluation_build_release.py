...
def test_metadata_emits_cargo_version_with_fixed_sequence_and_source_sha() -> None:
    ...
    assert values["sequence"] == "546"
    assert values["source_sha"] == SHA
    assert values["version"] == "0.1.1~dev.546+g0123456789ab"
    ...
    assert values["arm64_package"] == (
        "vonk-forge-agent_0.1.1~dev.546+g0123456789ab_arm64.deb"
    )
...
def test_same_sequence_distinguishes_source_sha() -> None:
    ...
    assert first.split("+", 1)[0] == second.split("+", 1)[0] == "0.1.1~dev.546"
...
def test_evaluation_version_sorts_between_physical_and_future_upstream() -> None:
    ...
    upstream_547 = "0.1.1~dev.547+gcccccccccccc"
    ordered = [physical_540, physical_544, evaluation, upstream_547]
