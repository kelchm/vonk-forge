...
def test_metadata_emits_cargo_version_with_fixed_sequence_and_source_sha() -> None:
    ...
    assert values["sequence"] == "609"
    assert values["source_sha"] == SHA
    assert values["version"] == "0.1.1~dev.609+g0123456789ab"
    ...
    assert values["arm64_package"] == (
        "vonk-forge-agent_0.1.1~dev.609+g0123456789ab_arm64.deb"
    )
...
def test_metadata_build_digest_matches_agent_package_v2_convention() -> None:
    ...
    bash = subprocess.run(
        [
            "bash",
            "-c",
            "printf 'vonk-agent-build-v2\\0%s\\0%s\\0%s\\0%s' \"$1\" linux-arm64 \"$2\" \"$3\" | sha256sum",
            "digest",
            SHA,
            lock_digest,
            version,
    ...
    assert values["build_digest"] == HELPER.v2_build_digest(
        SHA, "linux-arm64", lock_digest, version
    )
...
def test_same_sequence_distinguishes_source_sha() -> None:
    ...
    assert first.split("+", 1)[0] == second.split("+", 1)[0] == "0.1.1~dev.609"


def test_evaluation_version_sorts_after_deployed_local_package() -> None:
    evaluation = HELPER.evaluation_version("0.1.1", SHA)
    previous_local_608 = "0.1.1~dev.608+g61b3e4cc1adf"
    # These are ordering fixtures, not claims about a current upstream candidate.
    next_local_610 = "0.1.1~dev.610+gcccccccccccc"
    ordered = [previous_local_608, evaluation, next_local_610]
