        ...
        assembler.canonical(
            {
                "schema_version": 2,
                "channel": "dev",
                "version": "0.1.1~dev.609+g88480698a951",
...
def test_current_candidate_schema_requires_the_cli_wheel() -> None:
    names = assembler._manifest_artifact_names(assembler._candidate_release_model())

    assert "cli-wheel" in names
    assert "agent-package-linux-arm64" in names
    assert "nas-payload" in names


def test_cli_wheel_must_bind_this_source_and_version(tmp_path):
    source = "a" * 40
    version = "0.1.1~dev.609+g0123456789ab"
    wheel = tmp_path / "vonk_cluster_profiles-0.1.1-py3-none-any.whl"
    ...
    name, raw = assembler.cli_wheel_from(wheel, source, version)

    assert name == wheel.name
    assert raw == wheel.read_bytes()
    with pytest.raises(ValueError, match="build identity"):
        assembler.cli_wheel_from(wheel, "b" * 40, version)
    renamed = tmp_path / "not-a-wheel.bin"
    renamed.write_bytes(raw)
    with pytest.raises(ValueError, match="filename"):
        assembler.cli_wheel_from(renamed, source, version)
...
def test_spark_bootstrap_stages_versioned_package(tmp_path, signing_key):
    ...
    assert Path(args[1]).name == "vonk-forge-agent_0.1.1~dev.609+g88480698a951_arm64.deb"
