...
physical GPU acceptance. Old .540/.544 preinst blocks dpkg downgrade; restore
...
BASELINE_VERSION = re.compile(r"\A0\.1\.1~dev\.(540|544)\+g([0-9a-f]{12})\Z")
CANDIDATE_VERSION = re.compile(r"\A0\.1\.1~dev\.546\+g([0-9a-f]{12})\Z")
...
BASELINE_BUNDLE_SCHEMA = {
    ...
    "notes": [
        ...
        "Distinct from the candidate evaluation installer RSA key and package key.",
    ],
    "schema_version": 2,
    "version": "0.1.1~dev.540+gSOURCE12 or 0.1.1~dev.544+gSOURCE12",
}
CLEAN_ENV = {
...
def load_baseline(root: Path, expected_source: str) -> BaselineBundle:
    ...
    match = BASELINE_VERSION.fullmatch(version)
    digest, key_digest = (
    ...
    invalid = (
        ...
        or match is None
        or match.group(2) != expected_source[:12]
...
def load_current(bundle: Path, public_key: Path, candidate_source: str):
    ...
    match = CANDIDATE_VERSION.fullmatch(version)
    if (
        assembly.get("source") != candidate_source
        or match is None
        or match.group(1) != candidate_source[:12]
    ...
    return artifacts, version, generation
...
def recover(arguments: argparse.Namespace) -> dict[str, object]:
    ...
    artifacts, candidate_version, generation = load_current(
    ...
    live_key = live_sha256("/usr/share/keyrings/vonk-forge-release.pub")
    try:
        live_key_id = hashlib.sha256(
            bytes.fromhex(
                require(
                    ["/usr/bin/cat", "/usr/share/keyrings/vonk-forge-release.pub"],
                    sudo=True,
                )
                .decode()
                .strip()
    ...
    if (
        live_key == baseline.key_file_sha256
        or live_key_id == baseline.release_key_sha256
    ):
        raise LifecycleError("old and new identities are not distinct")
    checkpoint_json(
        restore_argv(
    ...
    require_quiescent(baseline.version, helper.SQLITE_PATHS)
    restored_agent = live_sha256("/usr/lib/vonk-forge/vonk-agent")
