...
def verify_local_signed_release(arguments: argparse.Namespace) -> LocalReleaseArtifacts:
    ...
    semantic_version = str(arguments.version).split("~", 1)[0].split("+", 1)[0]
    wheel_name = f"vonk_cluster_profiles-{semantic_version}-py3-none-any.whl"
    if WHEEL_NAME.fullmatch(wheel_name) is None:
        raise LifecycleError("evaluation CLI wheel identity is invalid")
    expected_paths = {
        ...
        "cli-wheel": f"{prefix}/cli/{wheel_name}",
    ...
    def observe(self) -> dict[str, object]:
        ...
        return {
            ...
            "evaluation": {
                ...
                "recipe_fixture_repository": "CarstVaartjes/vonk-forge-recipes",
